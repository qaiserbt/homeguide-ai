...
import { telHref as buildTelHref, smsHref as buildSmsHref, whatsAppHref as buildWhatsAppHref } from "../../utils/contactLinks";
...
export function AgentContactSheet({ agent, propertyAddress, open, onClose }: AgentContactSheetProps) {
  ...
  const telHref = buildTelHref(agent);
  const smsHref = buildSmsHref(agent, propertyAddress);
  const waHref = buildWhatsAppHref(agent, propertyAddress);
